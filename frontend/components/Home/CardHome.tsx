...
export const CardHome: React.FC<any> = (props) => {
    const router = useRouter();
    const [modalShow, setModalShow] = useState(false);
    return (
        <a>
            <Card
                className={`mb-4 ${styles[colors[props.value]]} rounded`}
                onClick={() => {
                    if (props.tag != 'Join Game') {
                ...
                <Row noGutters className="h-100">
                    ...
                    <Col
                        className={`d-flex align-items-center text-center ${styles['text']}`}>
                        {props.tag}
                    </Col>
                </Row>
            </Card>
